...
class DeltaScreenCapturer:
    def __init__(self, quality=50, scale=0.9, format="WEBP"):
        ...
        if format_upper not in ALLOWED_FORMATS:
            print(
                f"[!] Invalid format '{format}'. Allowed: {ALLOWED_FORMATS}. Defaulting to WEBP."
            )
            format_upper = "WEBP"
        ...
        # Rate-limiting for MSS recovery
        self._sct_retry_count = 0
        self._sct_last_retry = 0
        self._sct_cooldown = 5.0
        ...
        if self.scale < 1.0:
            ...
            if cv2:
                frame_array = cv2.resize(
                    frame_array, (new_w, new_h), interpolation=cv2.INTER_LINEAR
                )
            else:
    ...
    def _capture_raw(self):
        ...
        if self.sct:
            try:
                ...
                print(f"[-] MSS Error: {e}")
                # Rate-limited retry: Try to recreate sct if it permanently failed
                now = time.time()
                if self._sct_retry_count == 0 or (
                    now - self._sct_last_retry > self._sct_cooldown
                ):
                    try:
                        print("[*] Capture: Attempting to re-initialize MSS...")
                        self._sct_last_retry = now
                        self._sct_retry_count += 1
                        self.sct = mss.mss()
                        # If we get here, it worked!
                        self._sct_retry_count = 0
                        print("[+] Capture: MSS Re-initialized successfully.")
                    except Exception as re_e:
                        print(f"[-] Capture: MSS Re-initialization failed: {re_e}")
                        self.sct = None
    ...
    def _encode_delta(self, frame_array):
        ...
        for y in range(0, h, TILE_SIZE):
            for x in range(0, w, TILE_SIZE):
                ...
                if (
                    tile_key not in self.prev_hashes
                    or self.prev_hashes[tile_key] != tile_hash
                ):
                    # Tile changed
    ...
    def _compress_frame(self, frame_array):
        ...
        if self.use_gpu and cv2:
            ...
            _, buf = cv2.imencode(
                ".jpg", cv2.cvtColor(frame_array, cv2.COLOR_RGB2BGR), encode_param
            )
            return buf.tobytes()
        else:
            # CPU encoding
    ...
    def _compress_tile(self, tile_array):
        ...
        if self.use_gpu and cv2:
            encode_param = [cv2.IMWRITE_JPEG_QUALITY, self.quality]
            _, buf = cv2.imencode(
                ".jpg", cv2.cvtColor(tile_array, cv2.COLOR_RGB2BGR), encode_param
            )
            return buf.tobytes()
        else:
