...
class InputController:
    def __init__(self, screen_width=None, screen_height=None):
        ...
        print(f"[+] InputController (Native SendInput): Screen {self.screen_width}x{self.screen_height}")
    ...
    def move_mouse(self, x_norm, y_norm):
        ...
        if not self.screen_width: return
        ...
        ii_.mi = MouseInput(x, y, 0, MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, 0, ctypes.c_void_p(INJECTED_SIGNATURE))
    ...
    def press_key(self, key_code, pressed):
        ...
        debug_log(f"Press Key: VK={hex(vk) if vk else 'None'}, Pressed={pressed} (Sig: {hex(INJECTED_SIGNATURE)})")
    ...
    def type_text(self, text):
        ...
        for char in text:
            ...
            # Press modifiers
            if shift: self.press_key(0x10, True)
            if ctrl: self.press_key(0x11, True)
            if alt: self.press_key(0x12, True)
            ...
            # Release modifiers
            if alt: self.press_key(0x12, False)
            if ctrl: self.press_key(0x11, False)
            if shift: self.press_key(0x10, False)
