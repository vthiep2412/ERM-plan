        ...
        try:
            if nCode == HC_ACTION:
                ...
                try:
                    ...
                    extra_info = (
                        kb.dwExtraInfo
                        if isinstance(kb.dwExtraInfo, int)
                        else (kb.dwExtraInfo.value if kb.dwExtraInfo else 0)
                    )
                ...
                if (
                    self.log_callback
                    and not is_injected
                    and (wParam == WM_KEYDOWN or wParam == 0x0104)
                ):  # WM_SYSKEYDOWN
                    try:
                        ...
                        ret = ctypes.windll.user32.ToUnicode(
                            vk, scan, ctypes.byref(kbd_state), buff, 5, 0
                        )
        ...
        try:
            if nCode == HC_ACTION:
                ...
                try:
                    extra_info = (
                        ms.dwExtraInfo
                        if isinstance(ms.dwExtraInfo, int)
                        else (ms.dwExtraInfo.value if ms.dwExtraInfo else 0)
                    )
                    if extra_info == 0xFFC3C3:
                        is_injected = True
                except:
    ...
    def _message_loop(self):
        ...
        try:
            ...
            self._keyboard_hook = user32.SetWindowsHookExW(
                WH_KEYBOARD_LL, self._keyboard_proc, h_mod, 0
            )
