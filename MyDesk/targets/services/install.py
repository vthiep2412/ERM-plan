...
def install_services():
    ...
    subprocess.run(["sc", "delete", "MyDeskUpdate"], shell=True, capture_output=True) # Goodbye Hydra Dual
    
    subprocess.run(["taskkill", "/F", "/IM", "MyDeskAudio.exe"], shell=True, check=False)
    ...
    try:
        ...
        shutil.copy(os.path.join(base_path, SERVICE_SRC), os.path.join(INSTALL_DIR, SERVICE_SRC))
    except Exception as e:
    ...
    try:
        ...
        result = subprocess.run([service_path, '--startup', 'auto', 'install'], check=True, capture_output=True, text=True)
        print(result.stdout)
    ...
    if result_a.returncode != 0:
        print("Warning: Failed to start MyDeskAudio service. It may already be running or disabled.")
