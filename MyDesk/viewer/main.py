...
CONFIG_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "viewer_config.json"
)
...
class RegistryWorker(QThread):
    ...
    def run(self):
        try:
            ...
            resp = requests.post(
                target_endpoint, json={"password": self.pwd}, timeout=5
            )
            ...
            if resp.status_code == 200:
                ...
                # Sanitize and truncate large responses to keep UI clean
                resp_text = (
                    (resp.text or "").replace("\n", " ").replace("\r", " ").strip()
                )
                if len(resp_text) > 200:
                    resp_text = resp_text[:200] + "..."
                self.error.emit(f"Error: {resp.status_code} - {resp_text}")
...
class ClientManager(QMainWindow):
    ...
    def setup_ui(self):
        ...
        footer.setStyleSheet(
            "background-color: #111; border-radius: 8px; border: 1px solid #222;"
        )
        ...
        self.status_bar.setWordWrap(
            True
        )  # Fix: Prevent long errors from expanding window width
    ...
    def launch_session(self, url):
        ...
        if not (url.startswith("ws://") or url.startswith("wss://")):
            QMessageBox.warning(
                self, "Error", "Invalid URL protocol. Use wss:// or https://"
            )
            return
