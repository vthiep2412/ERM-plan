...
class FMTab(QWidget):
    ...
    def go_up(self):
        # Resolve '.' or empty paths to remote defaults (Drive list)
        if self.current_path == "." or not self.current_path:
            self.current_path = ""  # Show drive list
        ...
        if file_path:
            try:
                ...
                remote_path = ntpath.join(
                    self.current_path, os.path.basename(file_path)
                )
    ...
    def update_data(self, files, path=None):
        ...
        for row, file in enumerate(files):
            ...
            self.table.setItem(
                row, 1, QTableWidgetItem("📁 Folder" if is_dir else "📄 File")
            )
            self.table.setItem(
                row, 2, QTableWidgetItem(self.format_size(size) if not is_dir else "")
            )
