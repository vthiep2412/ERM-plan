...
class CurtainDialog(QDialog):
    curtain_selected = pyqtSignal(
        str, object
    )  # type ("BLACK", "IMAGE", "FAKE_UPDATE", "PRIVACY"), data (None or path)
    ...
    def load_saved_images(self):
        ...
        for filename in os.listdir(CURTAIN_DIR):
            ...
            if os.path.isfile(filepath) and filename.lower().endswith(
                ...
                icon = QIcon(
                    QPixmap(filepath).scaled(80, 60, Qt.AspectRatioMode.KeepAspectRatio)
                )
    ...
    def upload_image(self):
        filepath, _ = QFileDialog.getOpenFileName(
            self, "Select Curtain Image", "", "Images (*.png *.jpg *.jpeg *.bmp)"
        )
        if filepath:
            # Copy to curtain dir
