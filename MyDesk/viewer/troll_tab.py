            ...
            try:
                ...
                if size > 5 * 1024 * 1024:
                    self.show_error_message(
                        "File too large (>5MB). Please pick a smaller file."
                    )
                    return
                ...
                self.show_error_message(
                    f"Error reading audio file:\n{file_path}\n\n{e}"
                )

    def pick_and_send_video(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Video File", "", "Video Files (*.mp4 *.avi *.mkv *.webm)"
        )
        if file_path:
            try:
                ...
                self.show_error_message(
                    f"Error reading video file:\n{file_path}\n\n{e}"
                )

    def pick_and_send_wallpaper(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Wallpaper Image", "", "Image Files (*.jpg *.jpeg *.png *.bmp)"
        )
        if file_path:
            try:
                # Size check
